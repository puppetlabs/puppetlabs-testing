...
class testing (
  ...
  if $template {
    if $content {
      warning(translate('Both $template and $content parameters passed to mdlj, ignoring content'))
    }
  ...
  if $issue_template {
    if $issue_content {
      warning(translate('Both $issue_template and $issue_content parameters passed to mdlj, ignoring issue_content'))
    }
  ...
  if $issue_net_template {
    if $issue_net_content {
      warning(translate('Both $issue_net_template and $issue_net_content parameters passed to mdlj, ignoring issue_net_content'))
    }
  ...
  if $facts['kernel'] in ['Linux', 'SunOS', 'FreeBSD', 'AIX'] {
    ...
    if ($facts['osfamily'] == 'Debian') and ($dynamic_mdlj == false) {
